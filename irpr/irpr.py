...
def get_prob_line_by_date(df, date):
    ...
    for i, (r,y) in enumerate(zip(rates, y_data)):
        fig.add_trace(go.Scatter(x=x_data, y=y, mode='markers+lines',
                                name='<b>{}%</b><i style="font-size: 12;">/{:+}bps</i>'.format(r, int((r-baser)*100)),
    ...
    fig.update_layout(
        title=dict(text='Next Meeting Date: {}'.format(pd.to_datetime(date).strftime('%d-%b-%Y')), font_size=16),
    ...
    change = not all(tdf[[i for i in tdf.columns if '.' in i]].iloc[-2].fillna(0).rank()==tdf[[i for i in tdf.columns if '.' in i]].iloc[-1].fillna(0).rank())
...
def get_expected_line(df):
    ...
    df['datehour'] = df['created_at'].dt.date.astype(str) + '_' + df['created_at'].dt.hour.astype(str).str.zfill(2)
    ...
    i=0
    fig.add_trace(go.Scatter(x=x_data[i], y=y_data[i], mode='markers+lines', marker_symbol='hexagon',marker_size=6,
            name="{} (W{}, week ago)".format(datetime.strptime(labels[i], '%Y-%m-%d').strftime('%d-%b-%Y'), datetime.strptime(labels[i], '%Y-%m-%d').isocalendar().week),
            line=dict(color=colors[i], width=line_size[i], dash='dot'),
            connectgaps=True,
        ))
        
    i=1
    fig.add_trace(go.Scatter(x=x_data[i], y=y_data[i], mode='markers+lines', marker_symbol='hexagon',marker_size=10,
            name="{} (W{}, today)".format(datetime.strptime(labels[i], '%Y-%m-%d').strftime('%d-%b-%Y'), datetime.strptime(labels[i], '%Y-%m-%d').isocalendar().week),
            line=dict(color=colors[i], width=line_size[i]),
            connectgaps=True,
        ))
    
    dd = fig.data[i]
    fig.add_scatter(x=[dd.x[-1]], y = [dd.y[-1]],
                            mode = 'markers+text',
    ...
    fig.add_annotation(x=max_x, y=max_y, text=''.format(max_y), 
                       font=dict(size=16, color=colors[-1]), showarrow=False, arrowhead=2, arrowsize=1, arrowwidth=2)
    ...
    annotations.append(dict(xref='paper', yref='paper', x=0.5, y=1.05,
                                  ...
                                  text='Market Expectations for Fed Funds Rate<br>(Fed Funds Futures, {} - {})'.format(a,b),
    ...
    fig.update_layout(
        ...
        xaxis_range=[pd.to_datetime(x_data[0][0]).date()+timedelta(-30), pd.to_datetime(x_data[0][-1]).date()+timedelta(60)],
        xaxis=dict(
            dtick="M1",
            tickformat='%b-%y',
            showline=True,
...
def get_table(ldf):
    ...
    def _color_red_or_green(val): 
        ...
        colors = {-25:'#FDEDEC', -50:'#FADBD8', -75:'#F5B7B1', -100:'#F1948A',
                 25:'#EAFAF1', 50:'#D1F2EB', 75:'#ABEBC6', 100:'#82E0AA'}
    ...
    nldf.rename(columns={'GV1_DATE': 'Meeting Date', 'GEN_VAL3': 'Current Rate', 'expected_tp': 'Expected Rate'}, inplace=True)
